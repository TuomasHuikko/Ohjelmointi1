"""
COMP.CS.100 "ystäväverkko" tehtävä.
...
def add_friendship(network, name1, name2):
    ...
    if name2 not in network:
        network[name2] = []
    if name2 in network[name1] and name1 in network[name2]:
        pass
    else:
...
def common_friends_command(network, namelist):
    ...
    common_friends = []
    for name in sorted(network[name1]):
        if name == name1 or name == name2:
            continue
        elif name in network[name1] and name in network[name2]:
